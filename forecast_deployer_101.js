...
const DB_CONFIG = {
  ...
  user: process.env.DB_USER || "mariadb_user", // Replace with your MariaDB username or use .env
  password: process.env.DB_PASSWORD || "mariadb_password", // Replace with your MariaDB password or use .env
  database: process.env.DB_NAME || "database_name", // Replace with your database name or use .env
...
async function initDb() {
  try {
    ...
    //             abi LONGTEXT,
    //             bytecode LONGTEXT,
    //             forecast_game_abi LONGTEXT,
    //             forecast_game_bytecode LONGTEXT,
...
async function deployForecastFactory() {
  try {
    ...
    const forecastGameContractData =
      output.contracts["ForecastGame.sol"]["ForecastGame"];
    if (!forecastGameContractData) {
      throw new Error("ForecastGame contract not found in compilation output.");
    }
    ...
    const gameABI = JSON.stringify(forecastGameContractData.abi);
    const gameBytecode = "0x" + forecastGameContractData.evm.bytecode.object;
    ...
    /* Save ABI files to local directory
     */
    const deployedAbiDir = path.resolve(__dirname, "deployed_ABI");

    // 用当天日期作为子文件夹 (格式 YYYY-MM-DD)
    const today = new Date().toISOString().split("T")[0];
    const outputDir = path.join(deployedAbiDir, today);
    ...
    // 写入 factory-abi.json
    fs.writeFileSync(
      path.join(outputDir, "factory-abi.json"),
      JSON.stringify(factoryContractData.abi, null, 2),
      "utf8"
    );

    // 写入 game-abi.json
    fs.writeFileSync(
      path.join(outputDir, "game-abi.json"),
      JSON.stringify(forecastGameContractData.abi, null, 2),
      "utf8"
    );

    await logMessage("INFO", `ABI files saved to ${outputDir}`);

    /*
      Save all relevant details to the database
    */
